...
# Argument parsing
# ---------------------
parser = argparse.ArgumentParser(description="Run TabICL experiments.")
...
for filter_size in FILTER_SIZE:
    ...
    clf = TabICLClassifier(verbose=False,n_jobs=4, device=device)  # defaults to TabICLv2
    ...
    # Store results
    all_results.setdefault("TabICLClassifier", {
        "filter_sizes": [],
    ...
    res = all_results["TabICLClassifier"]
...
# Save results
# ---------------------
json_filename = f"all_tabicl_metrics_{target_col}_{dataset_flag}_{weighting_strategy}_{experiment_id}.json"
