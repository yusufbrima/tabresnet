...
import os
from models.nanotabpfn import NanoTabPFNClassifier, NanoTabPFNModel
import pandas as pd
from config import OUTPUT_PATH, TABULAR_EPOCHS, FILTER_SIZE, VAL_SIZE, TEST_SIZE, TARGET_COL, \
    PATIENCE, RESULT_PATH, CUTOFF, EICU_TARGETS, EICU_FILE
...
for filter_size in filter_sizes:
    ...
    models = {
        "NanoTabPFNModel": NanoTabPFNModel(embedding_size=96,num_attention_heads=4,mlp_hidden_size=192,num_layers=3,num_outputs=output_dim)
    }
    ...
    for name, model in models.items():
        ...
        for epoch in range(num_epochs):
            ...
            if val_loss < best_val_loss:
                ...
                patience_counter = 0
                best_epoch = epoch + 1
                torch.save(model.state_dict(), f'./modelstore/best_model_{name}_filter_{filter_size}_{experiment_id}.pth')
            else:
                patience_counter += 1
        ...
        # Load best model
        model.load_state_dict(torch.load(f'./modelstore/best_model_{name}_filter_{filter_size}_{experiment_id}.pth'))
